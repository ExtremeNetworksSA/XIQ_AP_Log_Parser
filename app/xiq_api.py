...
class XIQ:
    ...
    def sendCLI(self, device_id_list, cmds):
        ...
        if lro_url:
            lro_running = True
            count = 1
            print(f"Send CLI commands to {len(device_id_list)} devices using The long-running operation. Checking status in 30 secs.")
            t = 30
            while t:
                ...
                time.sleep(1)
                t -= 1
            while lro_running and count < 21:
                print(f"Attempting to collect CLI responses - attempt {count} of 20")
                try:
                ...
                if success:
                    if rawData['done'] == True:
                        ...
                        if rawData['metadata']['status'] != "RUNNING":
                            ...
                            print(f"The long-running operation is not complete. Checking again in 60 secs.")
                            logger.debug(f"LRO Status: {rawData['metadata']['status']}")
                            t = 60
                            while t:
                                mins, secs = divmod(t, 60)
                                timer = '{:02d}:{:02d}'.format(mins, secs)
